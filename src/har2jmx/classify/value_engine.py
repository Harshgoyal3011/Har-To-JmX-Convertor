...
def classify_values(cap: NormalizedCapture, lineage: LineageGraph | None = None) -> ClassificationResult:
    ...
    for flow in lineage.flows:
        ...
        if flow.first_producer is not None:
            ...
            # never produced by the server → first seen in a request → user input / master data
            ...
            if entity_name or _business_named(flow):
                ...
                reason = "client-supplied business/master data (varies per user)"
            else:
                cls, life, conf = ValueClass.UNKNOWN, Lifecycle.UNKNOWN, "Low"
                reason = "client-supplied value with no business/entity signal"
