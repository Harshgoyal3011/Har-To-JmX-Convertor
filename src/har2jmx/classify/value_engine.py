...
# "Bearer <token>" / "Basic <creds>" — the scheme wrapper around a credential. Lineage also exposes the
# bare credential as its own flow, so the wrapper is never itself a correlation target; flagging it just
# double-reports the token that flow already covers.
_SCHEME_CRED_RE = re.compile(r"^(?:Bearer|Token|Basic|Negotiate|Digest|NTLM|JWT|OAuth)\s+\S", re.IGNORECASE)
# client-generated OAuth/OIDC protocol nonces — the client mints them, so there is no issuing response to
# correlate; they are hardcoded or randomized, not a captured-secret gap.
_CLIENT_PROTOCOL_FIELDS = {"state", "nonce", "codechallenge", "codeverifier"}
...
def classify_values(cap: NormalizedCapture, lineage: LineageGraph | None = None) -> ClassificationResult:
    ...
    for flow in lineage.flows:
        ...
        if flow.first_producer is not None and not client_originated:
            ...
            client_field = _norm_field(min(req_occs, key=lambda o: o.request_index).field) if req_occs else ""
            if _SCHEME_CRED_RE.match(str(flow.value).strip()) or client_field in _CLIENT_PROTOCOL_FIELDS:
                # a "Bearer <token>" wrapper (the bare token is a separate flow, handled on its own) or a
                # client-minted OAuth nonce (state/nonce/PKCE) — neither is an uncaptured server secret.
                cls, life, conf = ValueClass.UNKNOWN, Lifecycle.UNKNOWN, "Low"
