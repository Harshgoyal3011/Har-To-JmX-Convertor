...
def _choose_extractor(location: str, cookie_name: str, consumers: list[Occurrence]) -> tuple[ExtractorType, str]:
    ...
    if location.startswith("response.meta:"):        # <meta name="csrf-token" content="..."> (SPA CSRF)
        field = location.split("response.meta:", 1)[1]
        return ExtractorType.REGEX, rf'name=["\']{re.escape(field)}["\'][^>]*content=["\']([^"\']*)["\']'
